    ...
    public double calculateTotalDiscount() {
      ...
      for (int combination = 0; combination < totalCombinations; combination++) {
        ...
        for (int i = 0; i < reservations.size(); i++) {
          ...
          int couponIndex = tempCombination % (availableCouponDtos.size() + 1);
          tempCombination /= (availableCouponDtos.size() + 1);

          CouponDto selectedCouponDto = null;

          if (couponIndex < availableCouponDtos.size()) {
            selectedCouponDto = availableCouponDtos.get(couponIndex);

            if (usedCoupons.contains(selectedCouponDto.getId())) {
            ...
            usedCoupons.add(selectedCouponDto.getId());
          }
