 ...
 * write 100 번 돌것이다
 * <p>
 * --spring.batch.job.names=sliceTestJob
 */
@Slf4j
@Configuration
...
public class SliceTestConfig {
  ...
  public static final String JOB_NAME = "sliceTestJob";
  public static final String STEP_NAME = "sliceTestStep";
  ...
  public Step couponUserMappingContributionMigrationStepV2() {
    return stepBuilderFactory.get(STEP_NAME)
      ...
      .reader(sliceCouponUserMappingContributionReader())
      .writer(sliceCouponUserMappingContributionWriter())
  ...
  @Bean
  @StepScope
  public ItemReader<List<Orders>> sliceCouponUserMappingContributionReader() {
    return () -> {
      Specification<Orders> spec = (root, query, criteriaBuilder) -> root.get("price").isNull();

      Slice<Orders> contents = repository.findAll(spec, PageRequest.of(0, PAGE_SIZE));
  ...
  @StepScope
  @Bean
  public ItemWriter<List<Orders>> sliceCouponUserMappingContributionWriter() {
    return items -> {
      List<Orders> list = items.get(0);
      list.forEach(orders1 -> orders1.setPrice(1000));
    };
  }
